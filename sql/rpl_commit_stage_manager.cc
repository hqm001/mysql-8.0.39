...
#endif
    while (thd->tx_commit_pending) {
      if (thd->thr_cond_lock_inited) {
        thd->tx_commit_signal_independently = true;
...
void Commit_stage_manager::signal_done(THD *queue, StageID stage [[maybe_unused]]) {
  size_t counter = 0;
  size_t signal_independent_counter = 0;
  ...
  for (THD *thd = queue; thd; thd = thd->next_to_commit) {
    ...
    counter++;
    if (thd->tx_commit_signal_independently) {
      signal_independent_counter++;
      thd->tx_commit_signal_independently = false;
      mysql_cond_broadcast(&thd->thr_cond_lock);
    }
  }

  if (counter != signal_independent_counter) {
    if (stage == COMMIT_ORDER_FLUSH_STAGE) {
